...
#from ssd300modelAll_alt import SSD
from ssd300small_alt import SSD
...
keys = sorted(gt.keys())
num_train = int(round(0.8 * len(keys))) #0.8
...
#model.load_weights('./checkpoints/weights.3.06-0.567.hdf5', by_name=True)


#layer_dict = dict([(layer.name, layer.output) for layer in model.layers])
for k in range(len(model.layers)):
    print("S",k,"    ",model.layers[k])
"""
...
for L in model.layers:
    if L.name in freeze:
        L.trainable = False
"""
for layer in model.layers[29:57]:
...
callbacks = [ csv_logger, weights_save, learnRateSchedule] #weights_save,
