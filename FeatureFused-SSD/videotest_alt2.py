...
class VideoTest(object):
    ...
    def run(self, video_path = 0, start_frame = 0, conf_thresh = 0.6):
        ...
        width, height = self.input_shape[0], self.input_shape[1]  #input_shape
...
#cv_fourcc('X', 'V', 'I', 'D')
        ...
        while True:
            ...
            #入力(500,400,3)に対して、ValueError: Error when checking : expected input_1 to have shape (500, 400, 3) but got array with shape (400, 500, 3)のため、入れ替え
            ...
            #to_draw = cv2.resize(resized, (int(self.input_shape[0]*1), self.input_shape[1]))
            to_draw = cv2.resize(resized, (int(self.input_shape[1]*vidar), self.input_shape[0]))
            ...
            if len(results) > 0 and len(results[0]) > 0:
                ...
                for i in range(top_conf.shape[0]):
                    ...
                    cv2.rectangle(to_draw, text_top, text_bot, self.class_colors[class_num], -1)  #to_draw
            ...
            to_draw = cv2.resize(to_draw, (int(self.input_shape[0]*1), self.input_shape[1]))
            
            g_frame = cv2.cvtColor(to_draw, cv2.COLOR_BGR2GRAY)  #to_draw
